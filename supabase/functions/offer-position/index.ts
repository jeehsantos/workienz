...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Validate token using user client
    ...
    if (!roleData) {
      return new Response(JSON.stringify({ error: "Only contractors can offer positions" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!job_id || !employee_user_id) {
      return new Response(JSON.stringify({ error: "job_id and employee_user_id are required" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!contractorProfile) {
      return new Response(JSON.stringify({ error: "Contractor profile not found" }), {
        status: 404,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (job.status !== "private") {
      return new Response(
        JSON.stringify({
          error: "Only private jobs can be used for direct offers. Please post the job as private first.",
        }),
    ...
    if (!favorite) {
      return new Response(
        JSON.stringify({
          error: "This worker is not in your favorites. You can only offer positions to favorited workers.",
        }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }
    ...
    if (existingApp) {
      return new Response(JSON.stringify({ error: "An application already exists for this worker on this job." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const { data: contractorUser } = await supabase.from("profiles").select("full_name").eq("user_id", userId).single();
