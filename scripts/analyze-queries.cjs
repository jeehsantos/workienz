...
class QueryAnalysisEngine {
   ...
   */
  detectNPlusOnePatterns(content, lines, file) {
   ...
   */
  detectFullTableScans(content, lines, file) {
   ...
   * Detect SELECT * usage
   */
  detectSelectStar(content, lines, file) {
   ...
   */
  detectInefficientJoins(content, lines, file) {
   ...
   */
  detectMissingIndexes(content, lines, file) {
    // Look for frequently filtered columns that might need indexes
    const filterPatterns = [
      { pattern: /\.eq\(['"]status['"],/, column: 'status', severity: 'high' },
      { pattern: /\.eq\(['"]user_id['"],/, column: 'user_id', severity: 'high' },
      { pattern: /\.eq\(['"]contractor_id['"],/, column: 'contractor_id', severity: 'high' },
      { pattern: /\.eq\(['"]employee_id['"],/, column: 'employee_id', severity: 'high' },
      { pattern: /\.eq\(['"]job_id['"],/, column: 'job_id', severity: 'high' },
      { pattern: /\.ilike\(['"]location_city['"],/, column: 'location_city', severity: 'medium' },
      { pattern: /\.order\(['"]created_at['"],/, column: 'created_at', severity: 'medium' },
    ];
   ...
   */
  extractQuerySnippet(lines, lineNumber, context = 2) {
...
async function generateOptimizationReport() {
  ...
  Object.entries(issuesByType).forEach(([type, issues]) => {
    detailedReport += `## ${typeLabels[type]} (${issues.length})\n\n`;
  ...
  detailedReport += '**Tool**: Query Analysis Script (scripts/analyze-queries.cjs)\n';
