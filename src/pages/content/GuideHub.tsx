...
import { Search, ChevronRight, ArrowLeft, Filter, Info, Plane, MapPin, Home, Briefcase, Wallet, Users, Settings, FileText, Clock, Lock, GraduationCap } from "lucide-react";
type Journey = {
  id: string;
  title: string;
...
export default function GuideHub() {
  ...
  useEffect(() => {
    ...
    const loadArticleFromLink = async () => {
      setIsLoading(true);
      let articleQuery = supabase.from("articles").select("id, title, slug, summary, visa_type, user_stage, article_type, is_premium, created_at, journey_id, topic_id, content, content_blocks, is_published").eq("id", articleId);
      if (!isAdmin() && !isWriter()) {
        articleQuery = articleQuery.eq("is_published", true);
      }
      const {
        data: articleData
      } = await articleQuery.maybeSingle();
      if (!articleData) {
        setIsLoading(false);
        return;
      }
      if (articleData.journey_id) {
        const {
          data: journeyData
        } = await supabase.from("journeys").select("id, title, description, icon_name").eq("id", articleData.journey_id).maybeSingle();
        if (journeyData) {
          setSelectedJourney(journeyData);
        }
        const {
          data: topicData
        } = await supabase.from("topic_hubs").select("id, journey_id, title, description").eq("journey_id", articleData.journey_id).eq("is_active", true).order("display_order", {
          ascending: true
        });
        setTopics(topicData || []);
        const {
          data: articleListData
        } = await supabase.from("articles").select("id, title, slug, summary, visa_type, user_stage, article_type, is_premium, created_at").eq("journey_id", articleData.journey_id).eq("is_published", true).order("created_at", {
          ascending: false
        });
  ...
  const fetchJourneys = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from("journeys").select("id, title, description, icon_name").eq("is_active", true).order("display_order", {
      ascending: true
    });
    if (error) {
      ...
      // Get topic counts
      const journeysWithCounts = await Promise.all((data || []).map(async journey => {
        const {
          count
        } = await supabase.from("topic_hubs").select("*", {
          count: "exact",
          head: true
        }).eq("journey_id", journey.id).eq("is_active", true);
        return {
          ...journey,
          topic_count: count || 0
        };
  ...
  const checkSubscription = async () => {
    ...
    const {
      data
    } = await supabase.from("subscriptions").select("id").eq("user_id", user.id).eq("status", "active").maybeSingle();
  ...
  const handleSelectJourney = async (journey: Journey) => {
    ...
    const {
      data: topicData
    } = await supabase.from("topic_hubs").select("id, journey_id, title, description").eq("journey_id", journey.id).eq("is_active", true).order("display_order", {
      ascending: true
    });
    setTopics(topicData || []);
    ...
    const {
      data: articleData
    } = await supabase.from("articles").select("id, title, slug, summary, visa_type, user_stage, article_type, is_premium, created_at").eq("journey_id", journey.id).eq("is_published", true).order("created_at", {
      ascending: false
    });
  ...
  const handleSelectArticle = async (article: Article) => {
    ...
    const {
      data
    } = await supabase.from("articles").select("*").eq("id", article.id).single();
    setFullArticle(data);
    setView("article");
    setIsLoading(false);
    window.scrollTo(0, 0);
    ...
    return <div className="min-h-screen bg-background">
        ...
        <div className="bg-gradient-to-br from-primary/10 via-background to-secondary/10 border-b border-border/50">
          <div className="container-tight py-12">
            {user && <Button variant="ghost" asChild className="mb-6">
                <Link to="/dashboard">
        ...
        <div className="container-tight py-8">
          ...
          <div className="relative max-w-2xl mx-auto mb-12">
            ...
            <Input type="text" placeholder="Search for a question (e.g. 'How do I get an IRD number?')" className="w-full pl-12 pr-4 py-6 text-lg rounded-xl" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
          </div>
          ...
          {isLoading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            </div> : journeys.length === 0 ? <div className="text-center py-16 bg-card rounded-xl border border-border/50">
              ...
              {journeys.map(journey => <div key={journey.id} onClick={() => handleSelectJourney(journey)} className="group border border-border p-6 rounded-2xl hover:border-primary/50 hover:shadow-lg transition-all cursor-pointer bg-card">
  ...
  if (view === "topic" && selectedJourney) {
    return <div className="min-h-screen bg-background">
        <div className="container-tight py-8">
          <button onClick={handleBackToHub} className="flex items-center text-muted-foreground text-sm mb-6 hover:text-foreground transition-colors font-medium">
          ...
          <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="lg:sticky lg:top-24 space-y-6">
                <div>
                  ...
                  <div className="space-y-2">
                    {VISA_FILTERS.map(f => <button key={f.value} onClick={() => setVisaFilter(f.value)} className={`w-full text-left px-4 py-3 rounded-xl text-sm transition-all ${visaFilter === f.value ? "bg-primary text-primary-foreground font-bold shadow-md" : "text-muted-foreground hover:bg-muted"}`}>
                        {f.label}
                      </button>)}
                  </div>
                </div>

                {topics.length > 0 && <div className="p-4 bg-primary/5 rounded-2xl border border-primary/10">
            ...
            <div className="flex-1 space-y-4">
              ...
              {isLoading ? <div className="space-y-4">
                ...
                </div> : filteredArticles.length === 0 ? <div className="text-center py-20 border-2 border-dashed border-border rounded-3xl bg-muted/50">
                  <p className="text-muted-foreground font-medium">No articles found matching your criteria.</p>
                </div> : filteredArticles.map(article => {
              const isLocked = article.is_premium && !hasSubscription && !user;
              return <div key={article.id} onClick={() => !isLocked && handleSelectArticle(article)} className={`p-6 border border-border rounded-2xl bg-card hover:border-primary/50 hover:shadow-xl cursor-pointer transition-all group ${isLocked ? "opacity-75" : ""}`}>
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          {article.visa_type && article.visa_type !== "all" && <Badge variant="secondary" className="text-[10px]">
  ...
  if (view === "article" && fullArticle) {
    ...
    return <div className="min-h-screen bg-background">
        <div className="container-tight py-8 max-w-3xl mx-auto">
          <button onClick={handleBackToTopic} className="flex items-center text-muted-foreground text-sm mb-8 hover:text-foreground transition-colors font-medium">
          ...
          <div className="mb-10">
            <div className="flex items-center gap-3 mb-6">
              <span className="text-[10px] font-bold text-primary uppercase tracking-[0.2em] px-3 py-1 bg-primary/10 rounded-full">
                {fullArticle.article_type === "guide" ? "Step-by-Step Guide" : fullArticle.article_type === "checklist" ? "Checklist" : "Atomic Answer"}
              </span>
            ...
            {/* TL;DR Box */}
            {fullArticle.summary && <div className="bg-foreground text-background p-6 md:p-8 rounded-2xl mb-10 shadow-xl">
            ...
            <div className="space-y-6">
              {contentBlocks.length > 0 ? contentBlocks.map((block: any, idx: number) => {
              const formattedContent = formatMarkdownText(block.value || "");
              if (block.type === "heading") {
                return <h2 key={idx} className="text-xl md:text-2xl font-bold text-foreground pt-6 border-t border-border">
                        {block.value}
                      </h2>;
              }
              if (block.type === "warning") {
                return <div key={idx} className="flex gap-4 p-4 md:p-6 bg-amber-50 dark:bg-amber-950/30 rounded-2xl border border-amber-100 dark:border-amber-900 text-amber-900 dark:text-amber-100">
              ...
              if (block.type === "tip") {
                return <div key={idx} className="flex gap-4 p-4 md:p-6 bg-emerald-50 dark:bg-emerald-950/30 rounded-2xl border border-emerald-100 dark:border-emerald-900 text-emerald-900 dark:text-emerald-100">
              ...
              return <div key={idx} className="text-base md:text-lg text-muted-foreground leading-relaxed" dangerouslySetInnerHTML={{
                __html: formattedContent
              }} />;
            }) : fullArticle.content ? <div className="prose prose-slate dark:prose-invert max-w-none" dangerouslySetInnerHTML={{
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    ...
                    <button type="button" onClick={handleBackToTopic} className="text-primary underline underline-offset-4">
                      Go back to the previous step
                    </button>
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  {user && isEmployee() && <ReportProblemDialog articleId={fullArticle.id} userId={user.id} articleTitle={fullArticle.title} />}
                </div>
              </div>
            </div>
          </div>
