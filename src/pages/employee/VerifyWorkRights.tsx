  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="container-tight py-8 max-w-2xl mx-auto">
        ...
        <h1 className="text-2xl font-bold mb-2 font-display">Verify Work Rights</h1>
        <p className="text-muted-foreground mb-8">
          To apply for jobs on Workie, you must verify your right to work in New Zealand.
        </p>

        {/* Current Status Banner */}
        <div className={`rounded-xl border p-6 mb-8 ${statusConfig.color}`}>
          <div className="flex items-start gap-4">
            <StatusIcon className="w-6 h-6 mt-0.5 flex-shrink-0" />
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <h2 className="font-semibold text-lg">{statusConfig.label}</h2>
        ...
        {/* Submission Form — only show if user can submit */}
        {canResubmit && (
          <div className="bg-card rounded-xl border border-border/50 shadow-soft p-6 space-y-6">
            <h3 className="font-semibold text-lg font-display">Submit Verification</h3>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => setDeclaredStatus(v as DeclaredWorkStatus)}
              >
              ...
              <div className="relative">
                ...
                {selectedFile ? (
                  ...
                  <label
                    ...
                    className="flex flex-col items-center justify-center gap-2 p-8 rounded-lg border-2 border-dashed border-border hover:border-primary/50 hover:bg-muted/30 cursor-pointer transition-colors"
                  >
        ...
        {latestRequest && (
          <div className="mt-6 bg-card rounded-xl border border-border/50 shadow-soft p-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <span className="font-medium">
                  {WORK_STATUS_OPTIONS.find(o => o.value === latestRequest.declared_status)?.label}
                </span>
              </div>
              <div>
                <span className="text-muted-foreground">Submitted:</span>{" "}
                <span className="font-medium">
                  {format(new Date(latestRequest.created_at), "dd MMM yyyy HH:mm")}
                </span>
              </div>
              ...
              {latestRequest.reviewed_at && (
                <div>
                  ...
                  <span className="font-medium">
                    {format(new Date(latestRequest.reviewed_at), "dd MMM yyyy HH:mm")}
                  </span>
                </div>
              )}
            </div>
